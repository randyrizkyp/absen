...
class _AbsenMasukPageState extends State<AbsenMasukPage> {
  ...
  void _cekRadius() {
    if (_posisi == null) return;

    final lokasiPengguna = LatLng(_posisi!.latitude, _posisi!.longitude);
    const radiusBoleh = 50.0;
    final distance = const Distance();

    final titikKodeunit = LatLng(
      double.parse(globals.latKodeunit),
      double.parse(globals.lotKodeunit),
    );

    final titikTptlain = LatLng(
      double.parse(globals.latTptlain.toString()),
      double.parse(globals.lotTptlain.toString()),
    );

    final titikOpdlain = LatLng(
      double.parse(globals.latOpdlain.toString()),
      double.parse(globals.lotOpdlain.toString()),
    );

    final jarak1 = distance(lokasiPengguna, titikKodeunit);
    final jarak2 = globals.latTptlain != 0.0
        ? distance(lokasiPengguna, titikTptlain)
        : double.infinity;
    final jarak3 = globals.latOpdlain != 0.0
        ? distance(lokasiPengguna, titikOpdlain)
        : double.infinity;

    final dalamJangkauan = [
      jarak1,
      jarak2,
      jarak3,
    ].any((j) => j <= radiusBoleh);

    setState(() => _dalamRadius = dalamJangkauan);
  }
  ...
  Future<void> _ambilFotoDepanBelakang() async {
    if (!_dalamRadius) {
      setState(
        () => _status =
            "Anda berada di luar radius 50 meter dari lokasi yang diizinkan.",
      );
      return;
    }
    ...
    try {
      ...
      setState(() => _fotoDepan = compressedDepan);
  ...
  Future<void> _kirimAbsen() async {
    ...
    try {
      ...
      String encryptNip(String nip) => encrypter.encrypt(nip).base64;
      ...
      request.files.add(
        await http.MultipartFile.fromPath('foto_depan', _fotoDepan!.path),
      );
      request.files.add(
        await http.MultipartFile.fromPath('foto_belakang', _fotoBelakang!.path),
      );
